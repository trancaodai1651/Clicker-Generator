...
class FlexOrganizer implements OrganizerController {
  ...
  start() {
    document.title = this.locale === 'vi' ? 'Trình tạo khay & hộp chia' : 'Bin & Sorting Tray Generator';
    document.documentElement.lang = this.locale;
    document.documentElement.setAttribute('data-theme', this.theme);
    document.body.innerHTML = renderOrganizerShell(this.state, FONT_OPTIONS);
    this.viewport = document.getElementById('organizerViewport');
  ...
  private refreshSidebar() {
    ...
    const next = document.getElementById('organizerSidebar');
  ...
  private onSidebarClick = (event: Event) => {
    ...
    if (language) {
      ...
      document.documentElement.lang = this.locale;
      document.title = ORGANIZER_COPY[this.locale].title;
      this.refreshSidebar();
      return;
    }
    if (target.closest('#org-theme')) {
      ...
      document.documentElement.setAttribute('data-theme', this.theme);
      document.getElementById('flex-organizer')?.setAttribute('data-theme', this.theme);
      this.updateThemeColors();
      this.refreshSidebar();
      return;
    }
  ...
  private syncOutput(id: string, value: number) {
    const output = document.getElementById(`org-${id}-value`);
    if (output) output.textContent = String(Math.round(value * 10) / 10);
    for (const input of Array.from(document.querySelectorAll<HTMLInputElement>('input[data-param]'))) {
      if (input.dataset.param === id && input !== document.activeElement) input.value = String(value);
    }
  }
  ...
  private renderStatus(status: OrganizerUiState['status'], message?: string) {
    ...
    const element = document.getElementById('organizerStatus');
  ...
  private updateInfo(info: BuildResult['info']) {
    const values: Record<string, string> = {
      'org-info-bbox': `${info.bbox.join(' × ')} mm`,
      'org-info-compartments': String(info.compartments),
      'org-info-cell': `${info.cellInner[0].toFixed(1)} × ${info.cellInner[1].toFixed(1)} mm`,
      'org-info-volume': `${info.volumeCm3.toFixed(1)} cm³`,
  ...
  private refreshExportButtons() {
    const download = document.getElementById('org-download') as HTMLButtonElement | null;
    if (download) download.disabled = !this.state.hasMesh;
    const label = document.getElementById('org-downloadLabel') as HTMLButtonElement | null;
