...
export function createBlocksController(): BlocksController {
  ...
  const syncExportButtons = () => {
    ...
    const exportButton = document.getElementById('blocksExport') as HTMLButtonElement | null;
    const stlButton = document.getElementById('blocksExportStl') as HTMLButtonElement | null;
  ...
  function mountPreview(container: HTMLElement) {
    ...
    const loadSwitchAsset = async () => {
      try {
        const response = await fetch(`${import.meta.env.BASE_URL}assets/switch/mx/mx-switch.3mf`);
