...
export function saveProject() {
  ...
  const proj = {
    ...
    settings: {
      ...
      borderWidth: s.borderWidth, mergeTopFrame: s.mergeTopFrame, keepMeshesSeparate: s.keepMeshesSeparate, 
      tolerance: s.tolerance, stemTolerance: s.stemTolerance, switches: s.switches, keychain: s.keychain, 
      smoothing: s.smoothing, removeBg: s.removeBg, importMode: s.importMode, 
...
export async function loadProject(file: File, reprocessFn: () => void, rebuildFn: () => void, uiHandler: any) {
  try {
    ...
    store.set({
      ...
      smoothing: set.smoothing ?? store.get().smoothing, removeBg: set.removeBg ?? store.get().removeBg,
