...
export interface UiState {
  ...
  topThickness: number;
  imageDepth: number;
  imageMargin: number; // 👈 Thêm Khoảng đệm
  borderWidth: number; // 👈 Thêm Độ dày viền
  mergeTopFrame: boolean; // 👈 Thêm biến tùy chọn hợp nhất
  isFlatKeychain: boolean; // 👈 THÊM DÒNG NÀY: Trạng thái bật/tắt Móc khóa phẳng
  keepMeshesSeparate: boolean; // 👈 Thêm biến này
...
export interface UiCallbacks {
  ...
  onImageMargin(mm: number): void;
  onBorderWidth(mm: number): void;
  onMergeTopFrame(merge: boolean): void; // 👈 Thêm hàm callback
  onKeepMeshesSeparate(keep: boolean): void; // 👈 Thêm hàm này
  onIsFlatKeychain(isFlat: boolean): void; // 👈 THÊM DÒNG NÀY: Callback khi gạt công tắc
...
export function createUi(
  ...
  const margin = $<HTMLInputElement>('margin');
  margin.addEventListener('input', () => cb.onImageMargin(+margin.value));
  const borderwidth = $<HTMLInputElement>('borderwidth');
  borderwidth.addEventListener('input', () => cb.onBorderWidth(+borderwidth.value));
  // 👇 THÊM BẮT SỰ KIỆN NÚT CÔNG TẮC 👇
  const mergeTopFrame = $<HTMLInputElement>('mergeTopFrame');
  mergeTopFrame.addEventListener('change', () => cb.onMergeTopFrame(mergeTopFrame.checked));
  const keepMeshesSeparate = $<HTMLInputElement>('keepMeshesSeparate');
  keepMeshesSeparate.addEventListener('change', () => cb.onKeepMeshesSeparate(keepMeshesSeparate.checked));
  ...
  const isFlatKeychain = $<HTMLInputElement>('isFlatKeychain');
  isFlatKeychain.addEventListener('change', () => cb.onIsFlatKeychain(isFlatKeychain.checked));
  ...
  bindValInput('marginVal', margin, cb.onImageMargin); // 👈 Thêm
  bindValInput('borderwidthVal', borderwidth, cb.onBorderWidth); // 👈 Thêm dòng này
  ...
  function update(state: UiState) {
    ...
    margin.value = String(state.imageMargin);
    setVal('marginVal', state.imageMargin.toFixed(1) + ' mm');
    borderwidth.value = String(state.borderWidth);
    setVal('borderwidthVal', state.borderWidth.toFixed(1) + ' mm');
    ...
    const exportStlBtn = $<HTMLButtonElement>('exportStl');
    if (exportStlBtn) exportStlBtn.disabled = !state.hasParts || state.building;
    ...
    $<HTMLInputElement>('mergeTopFrame').checked = state.mergeTopFrame;
    $<HTMLInputElement>('keepMeshesSeparate').checked = state.keepMeshesSeparate;
    
    const keepMeshesRow = $('keepMeshesRow');
    if (keepMeshesRow) {
      keepMeshesRow.style.display = state.mergeTopFrame ? 'flex' : 'none';
    }

    // 🟢 ĐỒNG BỘ TRẠNG THÁI CÔNG TẮC MÓC KHÓA PHẲNG
    const isFlatEl = $<HTMLInputElement>('isFlatKeychain');
    if (isFlatEl) {
      isFlatEl.checked = !!state.isFlatKeychain;
    }

    // 🟢 ẨN BỚT MỤC CẤU HÌNH SWITCH KHI ĐANG Ở CHẾ ĐỘ MÓC KHÓA PHẲNG
    const sectionSwitch = $('sectionSwitch');
    if (sectionSwitch) {
      sectionSwitch.style.display = state.isFlatKeychain ? 'none' : 'block';
    }
  }
