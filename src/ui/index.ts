...
export function createUi(
  ...
  // 2. RENDER CÁC COMPONENT ĐỘNG VÀO VIEWPORT
  const viewport = $('viewport');
  if (viewport) {
    viewport.insertAdjacentHTML('beforeend', `
      <div id="loadingOverlay" class="loading-overlay" hidden><div class="loading-spinner"></div><div class="loading-text">Generating 3D model…</div></div>
  ...
  // 3. GẮN SỰ KIỆN TỪ BINDINGS MODULES
  bindGlobalEvents(cb);
  ...
  function update(state: UiState) {
    ...
    const setVal = (id: string, text: string) => { const el = $<HTMLInputElement>(id); if (document.activeElement !== el) el.value = text; };
    ...
    const fmtSigned = (v: number, dec: number) => (v > 0.0001 ? '+' : v < -0.0001 ? '−' : '') + Math.abs(v).toFixed(dec) + ' mm';
    ...
    // Cập nhật Keychain & Các Checkbox
    ...
    if ($('keychainAngleVal')) $('keychainAngleVal').textContent = `${Math.round((((kc.angleDeg % 360) + 360) % 360))}°`;
    ...
    // Cập nhật Shape Type Tabs & Select
    ...
    // Edit Mode UI
    document.querySelectorAll('.edit-mode-btn').forEach(b => b.classList.toggle('active', (b as HTMLElement).dataset.editmode === state.editMode));
    ...
    if ($('extrudePanel')) {
      if (state.editMode === 'extrude') {
        ...
        if (state.selectedParts.length > 0) {
          ...
          $('extrudeLevelLabel').textContent = state.selectedParts.length > 1 ? `${state.selectedParts.length} parts selected · Level: ${level.toFixed(1)}` : `Level: ${level.toFixed(1)}`;
        } else {
