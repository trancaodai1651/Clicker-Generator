...
const store = createStore<UiState>({
  ...
  imageDepth: 0.8,
  imageMargin: 1.2,
  borderWidth: 2.6, // 👈 Thêm giá trị viền mặc định bo đẹp (2.6mm)
  mergeTopFrame: false,
  keepMeshesSeparate: true, // 👈 Thêm dòng này // 👈 Thêm cấu hình mặc định
  tolerance: 0.4,
  stemTolerance: 0,
...
const ui = createUi(sidebarLeft, sidebarRight, statusEl, {
  ...
  onBorderWidth: (mm) => {
    store.set({ borderWidth: mm });
    debouncedRebuild();
  },
  // 👇 THÊM EVENT NÀY 👇
  onMergeTopFrame: (merge) => {
    store.set({ mergeTopFrame: merge });
    debouncedRebuild();
  },
  // 👇 THÊM CALLBACK NÀY 👇
  onKeepMeshesSeparate: (keep) => {
    store.set({ keepMeshesSeparate: keep });
    debouncedRebuild();
  },
  onWidth: (mm) => {
  ...
  onExportSTL: () => {
    if (!latestParts.length) return;
    downloadSTL(latestParts, 'clicker.stl');
    downloadCount += 1;
    if (downloadCount === 1) showLicenseModal();
    else showLicenseToast();
  },
  ...
  onImportMode: (mode) => {
    ...
    store.set({
      ...
      imageMargin: mode === 'text' ? 2.5 : 1.2,
      borderWidth: mode === 'text' ? 3.5 : 2.6, // 👈 Thêm dòng này để tự động set viền mượt tùy định dạng
    });
    reprocess();
  },
...
// 🟢 HÀM LẤY CHÍNH XÁC VÀ DUY NHẤT CÁC MÀU ĐANG CÓ Ở THANH BAR BÊN TRÁI
function getAvailableColorOptions(s: UiState): RGB[] {
  const barColors: RGB[] = [];

  // 1. Lấy màu Body
  if (s.bodyColorRgb) {
    barColors.push(s.bodyColorRgb);
  }

  // 2. Lấy danh sách các màu Palette đang dùng (Slot 1, 2, 3, 4...)
  if (s.palette) {
    s.palette.forEach((p) => {
      if (p.filamentRgb) barColors.push(p.filamentRgb);
    });
  }

  // 3. Lọc bỏ các màu bị trùng lặp
  const uniqueColors: RGB[] = [];
  const seen = new Set<string>();

  for (const rgb of barColors) {
    const key = rgb.join(',');
    if (!seen.has(key)) {
      seen.add(key);
...
viewer.onPartPick((index, clientX, clientY, shiftKey) => {
  ...
  if (s.editMode === 'color') {
    ...
    if (!target) return;
    const options: RGB[] = getAvailableColorOptions(s);
...
function applyModelRecolor(target: ColorTarget, rgb: RGB, partIndex: number) {
  ...
  if (target.kind === 'region') {
    ...
    if (partIndex >= 0 && latestParts[partIndex]) {
      // 🟢 CLICK TRỰC TIẾP TRÊN MÔ HÌNH 3D: Chỉ đổi màu ĐÚNG đối tượng được chọn (ví dụ: chỉ lá cờ)
      const part = latestParts[partIndex];
      viewer.setPartColor(partIndex, rgb);
      latestParts[partIndex] = { ...latestParts[partIndex], colorRgb: rgb };
      overrides[part.name] = rgb;
    } else {
      // 🔵 ĐỔI MÀU TỪ THANH BAR BÊN TRÁI: Đổi màu đồng loạt cho toàn bộ nhóm cùng màu
...
function rebuild(quiet = false) {
  ...
  const params: BuildParams = {
    ...
    imageMargin: s.imageMargin, 
    borderWidth: s.borderWidth,
    mergeTopFrame: s.mergeTopFrame, // 👈 Truyền biến này vào Web Worker
    keepMeshesSeparate: s.keepMeshesSeparate, // 👈 Thêm dòng này
    isFlatKeychain: s.isFlatKeychain,
    capProud: 4.0,
...
function saveProject() {
  ...
  const proj = {
    ...
    settings: {
      ...
      imageDepth: s.imageDepth,
      imageMargin: s.imageMargin, // 👈 Thêm
      borderWidth: s.borderWidth, // 👈 Thêm
      mergeTopFrame: s.mergeTopFrame, // 👈 Thêm
      keepMeshesSeparate: s.keepMeshesSeparate, // 👈 Bổ sung dòng này
      tolerance: s.tolerance,
      stemTolerance: s.stemTolerance,
      switches: s.switches,
      keychain: s.keychain,
...
async function loadProject(file: File) {
  try {
    ...
    store.set({
      ...
      imageMargin: set.imageMargin ?? store.get().imageMargin, // 👈 Thêm
      borderWidth: set.borderWidth ?? store.get().borderWidth, // 👈 Thêm
      mergeTopFrame: set.mergeTopFrame ?? false, // 👈 Thêm
      keepMeshesSeparate: set.keepMeshesSeparate ?? true, // 👈 Bổ sung dòng này
