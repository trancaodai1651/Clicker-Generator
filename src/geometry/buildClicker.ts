...
export function buildClicker(
  ...
  if (params.baseShape === 'outline') {
    ...
    // 🟢 TỰ ĐỘNG TÍNH BÁN KÍNH BO THEO PADDING (BORDER):
    // Khi Padding = 0, smoothingRadius = 0 -> Viền sẽ ôm sát khít rịt vào mép ảnh
    const smoothingRadius = Math.min(4.0, border * 1.5);
    
    if (smoothingRadius > 0.05) {
      plate = simp(track(solidPlate.offset(smoothingRadius, 'Round', 2.0, 24).offset(-smoothingRadius, 'Round', 2.0, 24)), 0.05);
    } else {
      plate = simp(solidPlate, 0.05);
    }
  } else {
  ...
  for (const { r } of ordered) {
    ...
    // Giữ đúng độ cao phẳng nguyên bản, không cộng zOffset
    ...
    // Luôn xuất từng chi tiết thành các Part riêng biệt và GIỮ NGUYÊN MÀU GỐC (r.filamentRgb)
  ...
  const isMerged = (params as any).mergeTopFrame;

  // 🔴 NẾU KHÔNG BẬT MERGE: Đục lỗ trên mặt Base để ghép khớp nối
  // 🟢 NẾU BẬT MERGE: Bỏ qua đục lỗ -> Giữ Base là tấm đặc phẳng
  if (!isMerged) {
  ...
  // Chỉ gắn chân cắm Switch (stem) nếu KHÔNG PHẢI là Móc khóa phẳng
  if (!isFlatKeychain) {
    for (const st of stemAts) base = track(base.add(st));
  }

  // Chỉ tạo vành viền skirt nếu KHÔNG PHẢI là Móc khóa phẳng
  if (!isFlatKeychain && skirtLen > 0.4) {
  ...
  if (kc && kc.enabled) {
    ...
    // 🟢 Chọn vị trí mép gắn tai móc: Móc khóa phẳng gắn vào plate, Clicker gắn vào bodyFootprint
    const targetFootprint = isFlatKeychain ? plate : bodyFootprint;
    const { p, dir } = edgePointAt(targetFootprint, kc.angleDeg ?? 90);
    ...
    let loopFootprint = track(localFp.translate([px, py]));

    // 🟢 CẮT BỎ HOÀN TOÀN PHẦN CẦU NỐI LẸM VÀO VÙNG HÌNH ẢNH
    if (isFlatKeychain) {
      loopFootprint = track(loopFootprint.subtract(imageArea));
    }

    // Độ cao & tọa độ Z của tai móc
    ...
    let loop = extrudeAt(loopFootprint, loopTh, loopZb);

    const rr = (rotDeg * Math.PI) / 180;
    const hcx = -outward * Math.sin(rr) + px;
    const hcy = outward * Math.cos(rr) + py;
    const hole = extrudeAt(track(CrossSection.circle(holeR, 48).translate([hcx, hcy])), loopTh + 2, loopZb - 1);

    if (isFlatKeychain) {
      // 🟢 MÓC KHÓA PHẲNG: Đúc tai móc & đục lỗ trực tiếp trên tấm nền Base
      base = track(base.add(loop));
      base = track(base.subtract(hole));
    } else {
      // Clicker thông thường: Đúc tai móc vào thân Body dưới
  ...
  // 🟢 Nếu là Móc khóa phẳng: Bỏ qua không xuất phần vỏ Body dưới
  if (!isFlatKeychain && !body.isEmpty()) {
